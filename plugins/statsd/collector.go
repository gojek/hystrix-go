...
// stats to a Statsd backend. To use users must call InitializeCollector before
...
// CollectorConfig provides configuration that the Statsd client will need.
...
// InitializeCollector creates the connection to the Statsd server
// and should be called before any metrics are recorded.
//
// Users should ensure to call Close() on the client.
func InitializeCollector(config *CollectorConfig) (*CollectorClient, error) {
...
func (s *CollectorClient) NewStatsdCollector(name string) metricCollector.MetricCollector {
	if s.client == nil {
...
func (g *Collector) setGauge(prefix string, value int64) {
...
func (g *Collector) updateTimingMetric(prefix string, i int64) {
